"""
Arno 中文简化版 - AI 雅思/托福/多邻国 口语写作批改后端
核心功能：写作批改 / 口语批改，支持雅思/托福/多邻国
核心特点：在考生原文基础上逐句批改优化，一步一步教提分，保留考生原有内容
"""
...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS corrections (
            ...
            content_type TEXT,  # writing / speaking
            question TEXT,
            content TEXT,
            transcript TEXT,
            correction TEXT,
...
def correct_writing():
    ...
    # 构建AI prompt - 按照Arno优秀特点：在考生原有内容基础上一步步优化，逐句批改
    prompt = f"""你是一个专业的雅思/托福写作考官，请批改这篇{exam_type}写作。

**核心批改规则（必须严格遵守）：**
1. 完全保留考生原文的整体结构和观点，不要全盘重写，只在考生原文基础上**逐句优化修改**
2. 每一处修改都要说明：原文哪里错了/可以更好，为什么错/为什么这么改，给出修改后的句子
3. 给原文整体打分，告诉考生优化后预期能拿到多少分，让考生看到提分空间
...
2. 原文得分 → 优化后预期得分
3. 优点：列出2-3个考生写得好的地方
4. 逐句批改优化（这是最重要的部分，必须每句都改到）：
   - 原句：xxx → 问题/可以改进：xxx → 改正：xxx → 说明：为什么这么改
   - 每一处都要给具体修改，不要只说问题不给出改正后的句子
5. 整体改进建议：告诉考生下次怎么练习能提分
6. 完整优化版本：把修改好的完整文章放在这里，考生可以直接对比看

请用中文回复，方便中国考生看懂，解释清楚为什么这么改，考生才能学会一步步提高。
"""
    ...
    try:
        ...
        # 保存批改记录
        cursor.execute('''
            INSERT INTO corrections (user_id, exam_type, content_type, question, content, correction, created_at)
            VALUES (?, ?, 'writing', ?, ?, ?, ?)
...
# --- 口语批改 ---
@app.route('/api/correct/speaking', methods=['POST'])
def correct_speaking():
    auth_header = request.headers.get('Authorization')
    token = auth_header.replace('Bearer ', '')
    user_id = verify_token(token)
    if not user_id:
    ...
    exam_type = request.form.get('exam_type', 'ielts')  # ielts / toefl / duolingo
    question = request.form.get('question', '')  # 题目
    audio_file = request.files.get('audio', None)  # 录音文件
    
    if not question.strip():
        return jsonify({'code': 400, 'msg': '请输入题目要求'})
    
    if not audio_file:
        return jsonify({'code': 400, 'msg': '请上传录音'})
    ...
    # 保存音频文件
    audio_path = f'./audio/{user_id}_{datetime.now().strftime("%Y%m%d%H%M%S")}.wav'
    ...
    try:
        ...
        return jsonify({'code': 500, 'msg': f'语音转文字失败: {str(e)}'})
    
    # 构建AI prompt - 和写作一样，逐句批改优化
    prompt = f"""你是一个专业的雅思/托福口语考官，请批改这份{exam_type}口语回答。

**核心批改规则（必须严格遵守）：**
1. 完全保留考生回答的整体内容和观点，只在考生原文基础上**逐句优化修改**
2. 每一处修改都要说明：原文哪里错了/可以更好，为什么错/为什么这么改，给出修改后的句子
3. 给原文整体打分，告诉考生优化后预期能拿到多少分，让考生看到提分空间
...
2. 原文得分 → 优化后预期得分
3. 流利度与连贯性：评价和建议
4. 发音：评价发音，指出问题，给出改进建议
5. 逐句批改优化（这是最重要的部分）：
   - 原句：xxx → 问题/可以改进：xxx → 改正：xxx → 说明：为什么这么改
   - 每一处都要给具体修改，不要只说问题不给出改正
6. 整体改进建议：告诉考生下次怎么练习能提分
...
"""
    ...
    try:
        ...
        # 保存批改记录
        cursor.execute('''
            INSERT INTO corrections (user_id, exam_type, content_type, question, content, transcript, correction, created_at)
            VALUES (?, ?, 'speaking', ?, ?, ?, ?, ?)
        ''', (user_id, exam_type, question, transcript, correction, datetime.now()))
        conn.commit()
...
def get_history():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT id, exam_type, content_type, question, created_at 
        FROM corrections 
        WHERE user_id = ? 
