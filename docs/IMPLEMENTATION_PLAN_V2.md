...
### 2.3 FocusModal 支持三种窗口模式

| 模式 | 实现方式 |
|------|---------|
| 全屏 | backdrop-blur + 居中卡片 |
| 窗口化 | position: fixed + draggable + always-on-top |
| 最小化 | 隐藏 Modal，只在系统托盘显示 |

**模式切换状态** 存储在 `useAppStore().focusWindowMode = 'fullscreen' | 'window' | 'minimized'`
...
| # | 功能 | 测试步骤 | 验收标准 |
|---|------|---------|---------|
| 1 | 时间轴初始化 | 打开 Timeline 页面 | 默认滚动到当天当前时间位置 |
| 2 | 时间块显示 | 查看已有数据 | 每个时间块显示正确的开始/结束时间、分类、标题 |
| 3 | 点击编辑 | 点击任意时间块 | 打开 DetailPanel，所有字段可编辑，保存后生效 |
| 4 | 删除时间块 | 在 DetailPanel 点击删除 | 弹出确认 → 删除成功 → 时间块消失 → Toast 反馈 |
| 5 | 拖拽创建时间块 | 在空白处拖拽 | 创建新时间块 → 保存到数据库 → Toast 反馈 |
| 6 | 拖拽调整位置 | 拖动时间块上下移动 | 新位置生效 → 保存到数据库 → Toast 反馈 |
| 7 | 拖拽调整时长 | 拖动时间块上下边缘 | 新时长生效 → 保存到数据库 → Toast 反馈 |
| 8 | 更改分类 | 点击时间块的分类标签 | 分类变更 → 颜色更新 → Toast 反馈 |
| 9 | 任务侧边栏显示 | 查看右侧待办 | 显示所有未完成任务，数量正确 |
| 10 | 拖拽任务到时间轴 | 把任务拖到时间块 | 创建关联时间块 → 任务状态更新 → Toast 反馈 |
| 11 | 今日时长统计 | 查看顶部数字 | 今日总时长 = 所有时间块时长之和，数字正确 |

### 4.2 所有操作必须有 Toast 反馈

**统一的 Toast 消息模板**：

| 操作 | 成功消息 | 失败消息 |
|------|---------|---------|
| 创建时间块 | ✅ "已创建时间块" | ❌ "创建失败，请重试" |
| 更新时间块 | ✅ "已保存" | ❌ "保存失败，请重试" |
| 删除时间块 | ✅ "已删除时间块" | ❌ "删除失败" |
| 更改分类 | ✅ "已更改分类" | ❌ "更改失败" |
| 任务转时间块 | ✅ "已开始专注此任务" | ❌ "创建失败" |
...
export function useToastFeedback() {
  ...
  return {
    ...
    loading: (message: string) => toast({
      description: `⏳ ${message}`,
      duration: Infinity, // 需要手动 dismiss
    }),
    confirm: async (message: string, action: () => Promise<void>) => {
      // 带确认的操作
    }
  }
}
...
| 页面 | 需要添加 Toast 的操作 |
|------|----------------------|
| Dashboard | 开始专注、暂停专注、完成专注、放弃专注 |
| Tasks | 新建任务、编辑任务、删除任务、批量操作、标记完成 |
| Timeline | 创建/编辑/删除时间块、更改分类、拖拽任务 |
| Settings | 保存设置、清除数据、添加/删除分类 |

### 5.3 按钮状态反馈

- ✅ 所有异步操作期间按钮必须 disabled + 显示 loading spinner
- ✅ 操作成功后按钮颜色短暂变绿
- ✅ 操作失败后按钮颜色短暂变红
...
2. [ ] `npm run lint` 零错误、零警告
3. [ ] `npx tsc --noEmit` 零错误
4. [ ] 所有集成测试通过
5. [ ] 所有 E2E 测试通过
6. [ ] UI/UX 审查通过
7. [ ] Reviewer 代码审查通过
8. [ ] 控制台零错误（打开 DevTools Console 浏览所有页面 5 分钟无报错）
